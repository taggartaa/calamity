...
class Layer(Object):
    ...
    This allows you to group on screen objects and allows for relative placement.
    It will also dynamically grow to fit whatever is inside it.
    """
    def __init__(self, pos = (0,0), align="grid"):
        """
        @brief Constructs a Layer at specified position.
        
        @var position: A tuple of size 2 indicating the (x,y) coordinates of the layer (top left corner)
        ...
        self.__blendedBackground = False
    ...
    def blending(self, blended):
        ...
        """
        if blended == False and self.__blendedBackground == True:
    ...
    def add(self, item):
        ...
        if self.getBackgroundColor(False) != "" and self.getBackgroundColor(False) != "clear" and self._blended:
    ...
    def setBackgroundColor(self, color):
        ...
        if self._blended:
            for item in self.__items:
                item.blend(add=color, remove=self.getBackgroundColor(blended=False))
                self._blendedBackground = True
            
        # The old method can carry on from here
        Object.setBackgroundColor(self, color)
        
    def blend(self, add="", remove=""):
        """
        @brief Overloaded blend to propagate blend to all children of all sub-layers.
